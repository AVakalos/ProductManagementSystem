...
A new user can register providing username, email and password. The password will be stored encrypted in the database. The login to the system can be performed providing either email or username along with the password. The authentication mechanism will query the database for the specified user details and if the user exists and after successful credential identification the user is allowed to perform operations to the system.
...
- ##### ADMIN
 ...
 A user holding both roles can perform any operation to the products. By default when a new user is registered has the *USER* role. Currently, changes to the assignment of roles to users can be done only through the database tables, externally of the application (there is no API endpoint).
...
com.apostolis.spms  # Root package
    ...
    |-model         # Entities 
    |-repository    # Interfaces for data extraction from the database
    |-security      # Security configuration and JWT token implementation
    |-service       # User details and Product services
```
...
#### Security mechanism
The security mechanism to secure the REST endpoints of the appication is the JSON Web Token Security and is depicted in the diagram below. When a user is successfully logged in, the system generates a signed token with the user identity and roles and expiration time after 10 minutes. The token is returned to user by a HTTP response. A filter (JwtAuthFilter) is implemented for every incoming HTTP request which allows access to the secured REST endpoints only if the HTTP Request contains a valid token of previously authenticated (logged in) user. The validation process takes the user information from User Details Service and the token information and checks if the token is valid (not expired and not changed). The authorization checking is performed on method level in the Controller for every CRUD (Create Read Update Delete) operation. If the token is expired the user must re-login and use the new token.
...
To interact with the application you will need a client (e.g Postman) to send and recieve HTTP requests.
...
#### 2. Login
Login providing the credentials in a new POST request to the http://localhost:8080/api/auth/signin endpoint. You can use either email or username.
...
For example having the *USER* Role, you can add a new product to the system. Send a POST request to the http://localhost:8080/api/v1/products endpoint with the product details and the token in the Authorization tab as a Bearer token.
![Add Product](<README images/Add a new product.png>)

![Token](<README images/token.png>)
The complete set of operations is available in the [REST API Documentation](https://documenter.getpostman.com/view/29541731/2s9YC1XEmP).
